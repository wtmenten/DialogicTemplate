...
func get_global_info(key: String = "", default: Variant = null) -> Variant:
	var global_info := ConfigFile.new()
	var encryption_password := get_encryption_password()
	...
	if encryption_password.is_empty():
		...
		if global_info.load(SAVE_SLOTS_DIR.path_join('global_info.txt')) == OK:
			if key.is_empty():
				return _make_global_info_dict(global_info, 'main')['main']
			else:
...
func _make_global_info_dict(info_config_file: ConfigFile, section: String = ""):
	var dict = {}
	for s in info_config_file.get_sections():
		if section.length() > 0 and s != section: continue
		for key in info_config_file.get_section_keys(section):
			dict.get_or_add(section, {}).set(key, info_config_file.get_value(section, key))
	return dict
