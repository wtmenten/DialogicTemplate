...
func _reload_settings() -> void:
	...
	var global_info = dialogic.Save.get_global_info()
	for prop in dialogic.Save.get_global_info():
		if prop.begins_with('dialogic/settings'):
			settings[prop.trim_prefix('dialogic/settings/')] = global_info.get(prop)
...
func _set(property:StringName, value:Variant) -> bool:
	...
	if dialogic.has_subsystem('Save'):
		dialogic.Save.set_global_info('dialogic/settings/'+property, value)
	return true
